...
public class CunningDocumentScannerPlugin: NSObject,
    ...
    /// Launches the system camera, then the same manual corner cropper as gallery.
    func openCamera(from presentedVC: UIViewController) {
        withCameraAccess { [weak self] in
            guard let self = self else { return }

            guard UIImagePickerController.isSourceTypeAvailable(.camera) else {
                let errorMsg = "Camera is not available on this device"
            ...
            self.presentingController = picker
